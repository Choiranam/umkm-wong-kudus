...
const svgIcons = {
  ...
  Lainnya: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="M6 10.5a1.5 1.5 0 1 0 0 3a1.5 1.5 0 0 0 0-3m4.5 1.5a1.5 1.5 0 1 1 3 0a1.5 1.5 0 0 1-3 0m6 0a1.5 1.5 0 1 1 3 0a1.5 1.5 0 0 1-3 0"/></svg>`,
};
...
const MapResizeAndBounds = ({ points }) => {
  const map = useMap();
  ...
  useEffect(() => {
    const resizeAndFit = () => {
      ...
      if (points.length > 0) {
        ...
        map.setView([-6.8057, 110.8406], 12);
      }
    };

    resizeAndFit();
    const timer = setTimeout(resizeAndFit, 200);
...
const UMKMMap = () => {
  ...
  return (
    <div className="w-full h-[500px] rounded-2xl overflow-hidden shadow-2xl border-4 border-white">
      ...
      >
        <MapResizeAndBounds points={markerPoints} />
        <TileLayer
        ...
        {umkms.map((u) => {
          ...
          return (
            ...
            >
              <Popup maxWidth={380} closeButton={true}>
                <div className="p-3">
                  <Link to={`/detail-umkm/${u.slug}`} className="block">
                    <UMKMCard data={u} />
                  </Link>
                </div>
              </Popup>
